...
namespace Catch {
  namespace Generators {
    ...
    std::vector<size_t> randomiseIndices(size_t selectionSize,
                                         size_t sourceSize);
    ...
    auto random(size_t size) -> Generator<T> {
      return Generator<T>(
          size, pf::make_unique<GeneratorRandomiser<T>>(all<T>(), size));
    }
    ...
    struct Generators : GeneratorBase {
      ...
      auto operator[](size_t index) const -> T {
        ...
        CATCH_INTERNAL_ERROR("Index '" << index << "' is out of range ("
                                       << sizes << ')');
      }
    };
    ...
    auto generate(SourceLineInfo const& lineInfo, L const& generatorExpression)
      ...
      auto const& generator = static_cast<Generators<UnderlyingType> const&>(
  ...
  namespace Matchers {
    namespace Floating {
      ...
      WithinUlpsMatcher::WithinUlpsMatcher(double target, int ulps,
        ...
        CATCH_ENFORCE(ulps >= 0, "Invalid ULP setting: "
                                     << ulps << '.'
